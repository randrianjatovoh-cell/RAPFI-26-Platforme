...
async function checkAccess(req, res, next) {
  ...
  try {
    ...
    if (church) {
      if (user.fonction === 'Vérificateur') {
        if (church.federation !== user.federation) {
          return res.status(403).json({ error: 'Accès interdit à cette église (fédération)' });
        }
        return next();
      }
      if (user.fonction === 'Pasteur') {
        if (church.district !== user.district) {
          return res.status(403).json({ error: 'Accès interdit à cette église (district)' });
        }
        return next();
      }
    ...
    // L'église n'existe pas : seul un Pasteur peut la créer dans son district
    if (user.fonction === 'Pasteur') {
      req.newEglise = true;
      return next();
    }
