    ...
    if (db.isPostgres) {
      await db.run(
        `INSERT INTO gl_data (user_id, month, eglise, district, federation, sabbath_index, data)
         ...
         ON CONFLICT (month, eglise, sabbath_index) 
         DO UPDATE SET 
           data = EXCLUDED.data,
...
// ============================================================
// ✅ VOLA SISA TEO ALOHA - FONCTIONS CRUCIALES
// ============================================================
  ...
  try {
    ...
    if (db.isPostgres) {
      ...
      await db.run(
        ...
        month, cleanEglise, finalAmount, finalAmount
      );
    }
...
module.exports = {
  ...
  getFrais,
  setFrais,
  // ✅ FONCTIONS CRUCIALES - BIEN EXPORTÉES
  getVolaSisa,
  setVolaSisa,
  deleteAllDataForMonth,
  addUserLog,
