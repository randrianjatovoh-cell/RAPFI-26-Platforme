...
/**
 * 🔥 FONCTION AJOUTÉE : Met à jour n'importe quel champ du rapport
 * Si le rapport n'existe pas, il est créé automatiquement
 */
async function updateReportField(month, eglise, field, value) {
  const db = await openDb();
  // Vérifier si l'enregistrement existe déjà
  const existing = await db.get('SELECT 1 FROM monthly_reports WHERE month_id = ? AND eglise = ?', month, eglise);
  if (!existing) {
    // Créer un enregistrement vide
    await db.run(
      'INSERT INTO monthly_reports (month_id, eglise) VALUES (?, ?)',
      month, eglise
    );
  }
  // Construire la requête dynamique (le champ est validé dans la route, donc sûr)
  const sql = `UPDATE monthly_reports SET ${field} = ? WHERE month_id = ? AND eglise = ?`;
  await db.run(sql, value, month, eglise);
}
